...
for (let i = 0; i < conversation.length; i++){
    ...
    if (conversation[i].name === 'Unicorn') {
        ...
        itemElm.className = 'divUni'
        itemElm.innerHTML = conversation[i].text
        document.appendChild(itemElm)


    } else if (conversation[i].name = 'Cleverbot') {

        let itemElm = document.createElement('div');

        itemElm.className = 'divClev'
        itemElm.innerHTML = conversation[i].text
        document.body.appendChild(itemElm)
